document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentItemType = ''; // e.g., 'tournament', 'newsItem'
    let currentOperation = 'edit'; // 'edit' or 'create'
    ...
    function showSection(sectionId) {
        ...
        if (activeSection) {
            loadSectionContent(sectionId, activeSection); // Always reload content for dynamic updates
            activeSection.style.display = 'block';
            activeSection.classList.add('active');
        } else if (sectionId === 'dashboard') { // Special case for static dashboard
    ...
    function populateFormFields(type, item) {
        ...
        switch (type) {
            case 'tournament':
                ...
                if (item) { // Only show stats fields if editing an existing tournament
                    ...
                    formFieldsContainer.querySelector('#matchesPlayed')?.setAttribute('disabled', true); // Disable editing stats here
                    formFieldsContainer.querySelector('#goalsScored')?.setAttribute('disabled', true);
                }
                break;
            case 'tournamentStats': // Specific for editing stats directly
            ...
            case 'news':
                ...
                if (!item) { // Set publishedDate for new items
        ...
        // Disable stats fields if they exist in the general tournament edit form
    ...
    editForm.onsubmit = (event) => {
        ...
        if (currentOperation === 'create') {
            switch (currentItemType) {
                case 'tournament':
                    const newTournament = {
                        ...
                        matchesPlayed: 0, // Initial stats
                        goalsScored: 0,   // Initial stats
            ...
            switch (currentItemType) {
                case 'tournament':
                    ...
                    // Stats are handled by 'tournamentStats' type
                    break;
        ...
        modal.style.display = 'none';
        // Determine which section to refresh based on item type
        let sectionToRefresh = currentItemType.endsWith('Stats') ? currentItemType.slice(0, -5) + 's' : currentItemType + 's';
        if (currentItemType === 'news') sectionToRefresh = 'noticias'; // Ensure correct pluralization
    ...
    function createSelectElement(id, options, selectedValue, changeHandler) {
        ...
        if (changeHandler) { // The button will trigger the update, not the select itself
    ...
    function loadSectionContent(sectionId, sectionElement) {
        ...
        // Capitalize first letter and handle specific names
    ...
    if (initialSectionLink) {
        initialSectionLink.click(); // Simulate click to load dashboard and set active styles
    } else {
